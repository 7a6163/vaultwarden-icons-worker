...
function timedFetch(url: string, accept: string, timeoutMs: number): Promise<Response> {
  return fetch(url, {
    redirect: "follow",
...
async function collectIconLinks(htmlResponse: Response): Promise<IconCandidate[]> {
...
async function discoverCandidateUrls(domain: ParsedDomain, timeoutMs: number): Promise<string[]> {
  let html: Response;
  try {
...
async function fetchIcon(url: string, options: IconFetchOptions): Promise<IconResult | null> {
  ...
  const mime = (contentType ?? "image/x-icon").split(";")[0]?.trim() ?? "image/x-icon";
