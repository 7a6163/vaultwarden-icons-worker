...
describe("parseDomain", () => {
	...
	test("returns null for a malformed percent escape instead of throwing", () => {
		expect(parseDomain("/%E0%A4%A")).toBeNull();
	});

	test("returns null for an out-of-range port", () => {
		expect(parseDomain("/example.com:99999")).toBeNull();
	});
});
...
describe("isPublicHostname", () => {
	...
	test("rejects operator-supplied blocked suffixes", () => {
		expect(
			isPublicHostname("secret.corp.example.com", ["corp.example.com"]),
		).toBe(false);
